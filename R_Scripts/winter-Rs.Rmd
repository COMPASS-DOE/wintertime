...
```{r preliminary-graphs, echo = FALSE}
...
srdb_winter %>%
  dplyr::select(Record_number, Longitude, Latitude) -> winter_coords

# Extract srdb_winter location data from worldclim data for precip...
raster::extract(precip, winter_coords[2:3]) -> precip_coords
apply(precip_coords, 1, sum) -> map_winter
cbind(winter_coords, map_winter) -> map_coords


# ...and tmean
raster::extract(tmean, winter_coords[2:3]) -> tmean_vals
...
cbind(map_coords, mat_winter)  %>%
  ...
  mutate(mat_winter = mat_winter / 10)  %>% 
    distinct(Latitude, Longitude, .keep_all = TRUE) -> winter_points
